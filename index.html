...
<head>
    ...
    <title>Admin Dashboard | Future Interns</title>
...
<body class="gradient-bg text-slate-100 min-h-screen overflow-x-hidden">
    ...
    <div id="loginSection" class="flex items-center justify-center min-h-screen p-4">
        <div class="glass p-10 rounded-3xl shadow-2xl w-full max-w-md text-center">
            ...
            <div class="mt-10 pt-6 border-t border-slate-800">
                <p class="text-[10px] text-slate-500 uppercase font-bold tracking-[0.2em]">Master Access Credentials</p>
                <p class="text-sm font-mono text-blue-400/80 mt-2">Username: Admin@69</p>
                <p class="text-sm font-mono text-blue-400/80">Password: 123456789</p>
            </div>
        </div>
    </div>
    ...
    <div id="mainApp" class="hidden flex h-screen">
        <aside class="w-72 glass border-r border-white/5 p-8 flex flex-col">
            ...
            <nav class="space-y-4 flex-1">
                <button onclick="showPage('analytics')" id="btn-analytics" class="w-full text-left p-4 rounded-xl hover:bg-white/5 transition active-link text-sm font-semibold">Analytics</button>
                <button onclick="showPage('leads')" id="btn-leads" class="w-full text-left p-4 rounded-xl hover:bg-white/5 transition text-sm font-semibold">Lead List</button>
                <button onclick="showPage('contact')" id="btn-contact" class="w-full text-left p-4 rounded-xl hover:bg-white/5 transition text-sm font-semibold">Add New Lead</button>
            </nav>
            <button onclick="location.reload()" class="p-4 text-slate-500 hover:text-red-400 transition text-xs font-bold uppercase tracking-widest">Sign Out</button>
        </aside>
        ...
        <main class="flex-1 overflow-y-auto p-12 bg-transparent">
            <section id="page-analytics" class="page-content">
                <div class="mb-10">
                    ...
                    <p class="text-slate-400">Real-time conversion tracking and metrics[cite: 111].</p>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
                    <div class="glass p-8 rounded-2xl border-l-4 border-blue-500">
                        <p class="text-slate-500 font-bold text-xs uppercase tracking-widest">Total Leads</p>
                        <h3 class="text-5xl font-bold mt-2 text-white">2,481</h3>
                    </div>
                    <div class="glass p-8 rounded-2xl border-l-4 border-green-500">
                        <p class="text-slate-500 font-bold text-xs uppercase tracking-widest">Conversions</p>
                        <h3 class="text-5xl font-bold mt-2 text-green-400">42%</h3>
                    </div>
                    <div class="glass p-8 rounded-2xl border-l-4 border-purple-500">
                        <p class="text-slate-500 font-bold text-xs uppercase tracking-widest">Market Impact</p>
            ...
            <section id="page-leads" class="page-content hidden">
                <h1 class="heading-font text-4xl font-bold mb-8">Lead Database</h1>
            ...
            <section id="page-contact" class="page-content hidden">
                <div class="max-w-2xl mx-auto">
                    <h1 class="heading-font text-4xl font-bold mb-4">New Lead Entry</h1>
                    <p class="text-slate-400 mb-10">Commit a new prospective lead to the database[cite: 92].</p>
                    <div class="glass p-10 rounded-3xl space-y-6">
                        ...
                        <button onclick="addNewLead()" class="w-full bg-blue-600 hover:bg-blue-500 p-4 rounded-xl font-bold uppercase tracking-widest transition-all">Add Lead</button>
                    </div>
                </div>
            </section>
        </main>
    ...
    <script>
        ...
        function renderLeads() {
            ...
            container.innerHTML = leads.map(l => `
                <tr class="hover:bg-white/5 transition">
                    ...
                    <td class="p-6"><span class="px-3 py-1 rounded-full text-[10px] font-bold uppercase ${l.status === 'Converted' ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'}">${l.status}</span></td>
                    <td class="p-6 text-blue-400 text-sm cursor-pointer">Edit</td>
                </tr>
            `).join('');
        }
        ...
        function addNewLead() {
            ...
            if(name && email) {
                leads.push({ name, email, status: 'New' });
                renderLeads();
                showPage('leads');
            }
        }
